-- Public-safe RPC for creating a custom tour quote request.
-- Returns only the new id, so anon callers do not need SELECT on the table.
-- All field validation mirrors the existing INSERT RLS policy.
...
DECLARE
  new_id uuid;
BEGIN
  -- Validation matches the INSERT RLS policy WITH CHECK clause.
  ...
  INSERT INTO public.tour_custom_quote_requests(
    ...
    _num_guests, _num_days, _destinations, _destination_slug,
    _start_tour_date_id, _extras_slugs, _budget, _notes,
    _total_amount, _deposit_amount, COALESCE(_currency, 'EUR'), 'new', COALESCE(_payload, '{}'::jsonb)
  )
...
REVOKE ALL ON FUNCTION public.create_custom_quote_request(
  text, text, text, text, text, integer, integer, text[], text, uuid, text[], text, text, numeric, numeric, text, jsonb
) FROM public;
GRANT EXECUTE ON FUNCTION public.create_custom_quote_request(
  text, text, text, text, text, integer, integer, text[], text, uuid, text[], text, text, numeric, numeric, text, jsonb
